...
class Beauty_Company {
    ...
    public static function get_professional_company_id() {
        $user_id = get_current_user_id();
        ...
        return (int) $wpdb->get_var(
            $wpdb->prepare(
                "SELECT company_id FROM {$wpdb->prefix}beauty_professionals WHERE user_id = %d AND active = 1",
                $user_id
            )
        );
    }
    ...
    public static function get_current_company_id() {
        ...
        return self::get_professional_company_id();
    }
